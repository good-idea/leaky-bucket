...
export default class LeakyBucket {
  ...
  async throttle(cost = 1, append = true, isPause = false) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (append) {
        ...
        log(`Appended an item with the cost of ${cost} to the queue`)
      } else {
        this.queue.unshift(item)
        log(`Added an item to the start of the queue with the cost of ${cost} to the queue`)
        this.cleanQueue()
      }
  ...
  startTimer() {
    if (!this.timer && this.queue.length > 0) {
      ...
      log(`Processing an item with the cost of ${item.cost}`)

      this.refill()

      if (this.currentCapacity >= item.cost) {
        item.resolve()
        log(`Resolved an item with the cost ${item.cost}`)
        ...
        log(`Waiting ${timeToDelta} for topping up ${requiredDelta} capacity until the next item can be processed ...`)
  ...
  refill() {
    ...
    if (this.currentCapacity < this.capacity) {
      ...
      log(
        `Refilled the bucket with ${refillAmount}, last refill was ${this.lastRefill}, current Date is ${Date.now()}, diff is ${
      ...
      if (this.currentCapacity >= this.capacity) {
        ...
        this.lastRefill = null
        log(`Buckets capacity is fully recharged`)
      } else {
  ...
  cleanQueue() {
    ...
    if (index >= 0) {
      this.queue.splice(index).forEach((item) => {
        if (!item.isPause) {
          log(`Rejecting item with a cost of ${item.cost} because an item was added in front of it!`)
          item.reject(
            new Error(`Cannot throttle item because an item was added in front of it which caused the queue to overflow!`),
          )
  ...
  pauseByCost(cost: number) {
    this.stopTimer()
    log(`Pausing bucket for ${cost} cost`)
  ...
  pause(seconds = 1) {
    ...
    log(`Pausing bucket for ${seconds} seonds`)
   ...
   */
  drain() {
    log(`Draining the bucket, removing ${this.currentCapacity} from it, so that the current capacity is 0`)
  ...
  setTimeout(timeout: number) {
    log(`the buckets timeout is now ${timeout}`)
  ...
  setInterval(interval: number) {
    log(`the buckets interval is now ${interval}`)
  ...
  setCapacity(capacity: number) {
    log(`the buckets capacity is now ${capacity}`)
  ...
  updateVariables() {
    ...
    log(`the buckets max capacity is now ${this.maxCapacity}`)
    log(`the buckets refill rate is now ${this.refillRate}`)
  }
}
